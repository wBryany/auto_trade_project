...
- Binance 行情和订单/账户回报均使用 WebSocket，并带断线重连、listenKey 续期及重连快照对账。开仓明细优先聚合 `ORDER_TRADE_UPDATE`，按成交 ID 去重；缺失时单次 REST 补查，随后按 15、30、60 秒逐步退避至 5 分钟，补齐后停止查询。REST 仍用于启动/重连快照、交易写操作和异常结果确认。
- 开仓与交易所硬止损确认后，成交明细短暂未齐属于待对账；超过 5 分钟或实际查询异常会发“成交明细对账异常（已开仓）”，不会误报为下单失败，也不会重下单。IP 限制仍立即单独告警。HTTP 日志记录去除查询参数的接口路径、交易所返回的 IP 请求权重及封禁截止时间，便于排查共享代理出口的限流；WebSocket 不会消除交易写操作所需的 REST 限额。
- 限频等待由 HTTP 层执行，期间禁止向受限主机继续发包；引擎每 30 秒继续检查可用的 WebSocket 状态，不再整段休眠到封禁结束。REST 平仓在封禁解除前仍可能不可用，交易所硬止损持续保护；手动切换节点后，通过页面重启引擎解除旧出口的等待，再使用安全重启脚本部署新代码。
