...
超短反转分支可用 `traditional_ultra_short_reversal_allow_long` 和 `traditional_ultra_short_reversal_allow_short` 独立控制方向。关闭某一方只会抑制 1m 极值收复/拒绝触发的反转单，不会影响同方向的趋势延续与回调续走信号。
